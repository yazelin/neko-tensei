...
2. 決定要不要回頭重生。**prompt 那條已經寫進去了**（2026-08-06，`scripts/prompt.py`
   的 `RULES` 與 `REMINDER`：氣泡貼著 `from <name>` 標的那位、尾巴指向他；畫外音
   的氣泡放在畫面邊緣、尾巴指向畫外；同格兩人各站一邊）。**還沒驗證有沒有用**
   ——要等下一話產出，或挑一頁重生來看。重生一頁約 226 秒。
3. 重生的副作用要先知道：第六話第 05 頁重生後頭修好了，但版面從三格變兩格。
   同一份 prompt 重擲本來就會換一種構圖，不是只換有問題的地方
